...
namespace GRINS
{
  ConstantDamping::ConstantDamping( const std::string& physics_name, const GetPot& input )
  ...
  void ConstantDamping::damping_residual( bool compute_jacobian,
                                          ...
                                          CachedValues& /*cache*/ )
  {
    for( std::vector<VariableIndex>::const_iterator v_it = _vars.begin();
         v_it != _vars.end(); ++v_it )
      {
        ...
        libMesh::DenseSubMatrix<libMesh::Number> &C_var = context.get_elem_jacobian(var,var);
        ...
        for (unsigned int qp=0; qp != n_qpoints; qp++)
          {
            libMesh::Real var_dot;
            context.interior_rate( var, qp, var_dot );

            libMesh::Real jac = JxW[qp];

            for (unsigned int i=0; i != n_dofs; i++)
              {
                F_var(i) += (this->_value)*var_dot*phi[i][qp]*jac;

                if( compute_jacobian )
                  {
                    for (unsigned int j=0; j != n_dofs; j++)
                      {
                        libMesh::Real jac_term = this->_value*phi[i][qp]*phi[j][qp]*jac;
                        jac_term *= context.get_elem_solution_rate_derivative();
